...
class TradingViewTickerExtractor:
    ...
    def extract_tickers(self) -> pd.DataFrame:
        ...
        try:
            ...
            self.data = self.data.sort_values(['exchange', 'symbol']).reset_index(drop=True)

            return self.data
        except FileNotFoundError:
            raise FileNotFoundError(f"File not found: {self.file_path}")
        except json.JSONDecodeError as e:
            raise json.JSONDecodeError(f"Invalid JSON in {self.file_path}: {str(e)}", e.doc, e.pos)
    ...
    def filter_by_sector(self, sector: str) -> pd.DataFrame:
        ...
        return self.data[self.data['sector'].str.lower() == sector.lower()][['symbol', 'name', 'sector']]
