...
class TradesMixin:
    def trade(self, df, column, initial_amount):
        ...
        for date, row in df.iterrows():
            ...
            if signal == 1 and stock_quantity == 0:  # Buy
                ...
                trades.append({
                    ...
                    'exit_price': price,
                    'profit': profit,
                    'return_%': (profit / initial_amount) * 100 if initial_amount > 0 else 0
                })
            ...
            portfolio_value = cash + (stock_quantity * price if stock_quantity > 0 else 0)
        ...
        total_return = ((portfolio_value - initial_amount) / initial_amount * 100
                       if initial_amount > 0 else 0)
        success_rate = (df_trades['profit'] > 0).mean() * 100 if len(df_trades) > 0 else 0
...
class RSIBacktesting(TradesMixin):
    def _setup(self, df: pd.DataFrame, column: str, period: int, lower_limit: int, upper_limit: int):
    ...
    def backtest(self, df, column='Close', period=14, initial_amount=10000, lower_limit=30, upper_limit=70):
...
class CCIBacktesting(TradesMixin):
    ...
    def _setup(self, df: pd.DataFrame, close_column: str, cci_period: int, low_th: float, high_th: float,
               initial_capital: float, commission_rate: float = 0.001):
        ...
        # Cumulative
        df['Cumulative_Strategy'] = (1 + df['Strategy_Return'].fillna(0)).cumprod() * initial_capital
    ...
    def backtest(
        ...
        """
        # Setup
        self._setup(df, close_column, cci_period, low_th, high_th, initial_capital, commission_rate)
